    ...
    companion object {
        ...
        private fun buildDatabase(appContext: Context): AppDatabase {
            ...
            val instance = Room.databaseBuilder(
                ...
                // ترقيات حقيقية تحافظ على البيانات في كل المسارات، وكل نسخة
                ...
                // IllegalArgumentException جوه .build() نفسها - يعني التطبيق
                // كان بيقفل عند كل فتح، حتى على تركيب جديد. الـ salvage
                // migrations اللي فوق هي اللي بقت بديلة السطر ده أصلاً
                // (راجع LegacyMigrations.kt).
                //
                // الوحيدة اللي مفيش ليها مسار بطبيعتها هي النزول لنسخة أقدم
                // (المستخدم ثبّت APK أقدم فوق بياناته): ساعتها إعادة إنشاء
                // القاعدة أحسن من قفلة عند كل فتح، والسطر ده مبيتعارضش مع أي
                // migration لأنه بيشتغل بس لما نسخة الملف أعلى من نسخة الكود.
